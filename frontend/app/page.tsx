      ...
      <div className="relative z-10 w-full">
        ...
        <header className="fixed top-0 left-0 right-0 bg-white/95 shadow-sm px-6 py-4 flex items-center justify-between z-20 backdrop-blur-sm border-b border-gray-100">
          ...
          <nav className="flex items-center gap-7">
            ...
            <a href="mailto:wildansekolah11@gmail.com" className="bg-[#005D41] hover:bg-[#15803D] text-white font-semibold px-6 py-2 rounded-full text-sm transition-colors">
              Hubungi Kami
            </a>
          </nav>
        </header>
        ...
        <section className="flex flex-col items-center justify-start pt-40 pb-24 px-4 min-h-[90vh]">
          ...
          <div className="w-full max-w-2xl mb-10">
            ...
            <div className="mt-8">
              ...
              {error && !loading && (
                <div className="p-6 bg-red-50 border border-red-200 rounded-xl text-red-700 text-base text-center">
                  {error}
                </div>
              )}
                ...
                <div className="space-y-6">
                  ...
                  {result.query_summary && (
                    <div className="text-center">
                      <p className="inline-block bg-[#005D41]/10 text-[#005D41] text-sm font-medium px-4 py-1.5 rounded-full">
                        {result.query_summary}
                      </p>
                    </div>
                  )}

                  {/* Hasil Error dari backend */}
                  {result.error && (
                    <div className="p-6 bg-red-50 border border-red-200 rounded-xl text-red-700 text-base text-center">
                      {result.error}
                    </div>
                  ...
                  {result.results.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      {result.results.map((item) => (
                        <div
                          key={item.rank}
                          className="bg-white rounded-2xl border border-gray-200 shadow-md hover:shadow-lg transition-shadow overflow-hidden flex flex-col"
                        >
                          {/* Header Kartu */}
                          <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#005D41] text-white text-xs font-bold">
                                  {item.rank}
                                </span>
                                <h4 className="text-lg font-bold text-[#033B2B] truncate">{item.name}</h4>
                              </div>
                            ...
                            {item.categories.length > 0 && (
                              <div className="flex flex-wrap gap-1.5">
                                {item.categories.map((cat) => (
                                  <span key={cat} className="text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-md font-medium">
                                    {cat}
                                  </span>
                                ))}
                              </div>
                          ...
                          <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-xs text-[#005D41] hover:text-[#15803D] font-semibold transition-colors flex items-center gap-1"
                            >
                              Lihat di IG
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    /* Jika hasil kosong */
                    <div className="p-8 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl text-center">
                      <div className="text-4xl mb-3">🍽️</div>
                      <p className="text-gray-600 text-base">Belum ada rekomendasi yang cocok.</p>
                      <p className="text-gray-400 text-sm mt-1">Coba kata kunci lain atau lebih spesifik.</p>
                    </div>
                  )}
                </div>
              )}
